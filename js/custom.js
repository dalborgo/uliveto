...
(function ($) {
    ...
    $("#5stelle").submit(function (event) {
        event.preventDefault();

        var from = $("#bd").val().split("/");
        var f = from[2] + '-' + from[1] + '-' + from[0];
        var from2 = $("#ed").val().split("/");
        var f2 = from2[2] + '-' + from2[1] + '-' + from2[0];
        window.open("https://secure.kosmosol.it/booking/check-availability.php?h=panorama-torridelbenaco&l=it" + "&bd=" + f + "&ed=" + f2, '_blank');
    });
